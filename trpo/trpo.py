...
sys.path.append(os.environ['PROJ_HOME_3']+'/spinningup')
...
def trpo(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0, 
    ...
    # v_loss = tf.reduce_mean((v - ret_ph)**2)
    ...
    if main_kwargs['vf_load'] == "yes":
        if main_kwargs['vf_switch'] == "yes":
            v_update = True
    ...
    def update(val_update_criteron=False):
        # Prepare hessian func, gradient eval
        if not v_update:
            inputs = {k:v for k,v in zip(all_phs, buf.get())}
        else:
            inputs = {k:v for k,v in zip(all_phs, buf.get() + [val_update_criteron])}
    ...
    ep_mean_rews = list()        # this is for saving global info for multiple training results.
    ...
    for epoch in range(epochs):
        for t in range(local_steps_per_epoch):
            ...
            agent_outs = sess.run(get_action_ops, feed_dict={x_ph: o.reshape(1,-1), stochastic: True})
        ...
        if epoch_so_far % EVALUATION_FREQUENCY == 0:
            eval_max_iters = 5  # 5
            eval_timesteps_per_actorbatch = local_steps_per_epoch
            ...
            for eval_epoch in range(eval_max_iters):
                ...
                eval_ep_rets = []
                eval_sucs    = []
                for eval_t in range(eval_timesteps_per_actorbatch):
                    agent_outs = sess.run(get_action_ops, feed_dict={x_ph: o.reshape(1, -1), stochastic: False})
                    a, v_t, logp_t, info_t = agent_outs[0][0], agent_outs[1], agent_outs[2], agent_outs[3:]

                    o, r, d, ep_info = env.step(a)
                    eval_ep_ret += r
                    eval_ep_len += 1

                    terminal = d or (eval_ep_len == max_ep_len)
                    if terminal or (eval_t == local_steps_per_epoch - 1):
                        if not (terminal):
                            print('Warning: trajectory cut off by epoch at %d steps.' % eval_ep_len)
                        if terminal:
                            eval_lenbuffer.append(eval_ep_len)
                            eval_rewbuffer.append(eval_ep_ret)
                            eval_ep_rets.append(eval_ep_ret)
                            eval_ep_lens.append(eval_ep_ret)
        ...
        if epoch_so_far % MODEL_SAVING_FREQ == 0:
            # pi.save_model(main_kwargs['MODEL_DIR'], iteration=epoch_so_far)

            # save necessary training statistics
            with open(main_kwargs['RESULT_DIR'] + '/train_reward_' + 'iter_' + str(epoch_so_far) + '.pkl', 'wb') as f_train:
                pickle.dump(ep_mean_rews, f_train)

            # save necessary evaluation statistics
            with open(main_kwargs['RESULT_DIR'] + '/eval_success_rate_' + 'iter_' + str(epoch_so_far) + '.pkl', 'wb') as f_eval:
                pickle.dump(eval_success_rates, f_eval)

        """ Plotting and saving statistics """
        PLOT_FREQUENCY = 10 # 10
        if epoch_so_far % PLOT_FREQUENCY == 0:
            ...
            plot_performance(x=train_plot_x, y=train_plot_y, ylabel=r'episode mean reward at each iteration',
                             xlabel='ppo iterations', figfile=os.path.join(main_kwargs['FIGURE_DIR'], 'train_reward'), title='TRAIN')
            ...
            plot_performance(x=eval_plot_x, y=eval_plot_y,
                             ...
                             xlabel='ppo iterations', figfile=os.path.join(main_kwargs['FIGURE_DIR'], 'eval_success_rate'),
                             title="EVAL")
