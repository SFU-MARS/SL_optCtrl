...
# comment in case of changing track.world file accidently
...
#         track_data = mateng.load('/local-scratch/xlv/SL_optCtrl/MPC/LTV MPC Racing Reference Tracking with Boundary/track2.mat', nargout=1)
#         track_data = track_data['track2']
#         track_inner = np.array(track_data['inner'])
#         track_outer = np.array(track_data['outer'])
#
#     elif MODE == 'CSV':
#         track_data = pd.read_csv("/local-scratch/xlv/SL_optCtrl/MPC/track2_scaled.csv")
#         track_inner = np.array([track_data['inner_x'], track_data['inner_y']])
#         track_outer = np.array([track_data['outer_x'], track_data['outer_y']])
#
