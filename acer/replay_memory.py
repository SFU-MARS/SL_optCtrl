...
class ReplayBuffer:
    ...
    def sample(self, batch_size, window_length=float('inf')):
        ...
        """
        batched_trajectory = []
        # random choices is only available from python3.6+, so here we switch to python3.5
        # trajectory_indices = random.choices(range(len(self.episodes)-1), k=min(batch_size, len(self.episodes)-1))
        # print("length of episodes:", len(self.episodes))
        trajectory_indices = np.random.choice(range(len(self.episodes)), size=min(batch_size, len(self.episodes)))
        trajectories = []
        for trajectory in [self.episodes[index] for index in trajectory_indices]:
            # same as above
            # start = random.choices(range(len(trajectory)), k=1)[0]
            start = np.random.choice(range(len(trajectory)), size=1)[0]
        ...
        print("batched trajectory:", batched_trajectory)
