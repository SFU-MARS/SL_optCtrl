...
class ContinuousActorCritic(ActorCritic):
    ...
    def __init__(self):
        ...
        # ------ AMEND: initialized ACER ContinuousActorCritic structure ------ #
        STATE_SPACE_DIM = 13
        ACTION_SPACE_DIM = 2

        self.policy_input_layer = torch.nn.Linear(STATE_SPACE_DIM, 64)
        # print(list(self.policy_input_layer.parameters()))
        self.policy_hidden_layer = torch.nn.Linear(64, 64)
        self.policy_mean_layer = torch.nn.Linear(64, ACTION_SPACE_DIM)
        self.policy_logsd = torch.nn.Parameter(np.log(INITIAL_STANDARD_DEVIATION) * torch.ones((1, ACTION_SPACE_DIM)))
        ...
        self.policy_input_layer.weight.data = torch.from_numpy(np.transpose(weights[0]))
        self.policy_input_layer.bias.data = torch.from_numpy(weights[1])
        # print(self.policy_input_layer.parameters().dtype)
        # print(list(self.policy_input_layer.parameters()))
        self.policy_hidden_layer.weight.data = torch.from_numpy(np.transpose(weights[2]))
        self.policy_hidden_layer.bias.data = torch.from_numpy(weights[3])
        self.value_layer.weight.data = torch.from_numpy(np.transpose(weights[4]))
        self.value_layer.bias.data = torch.from_numpy(weights[5])
        # print("initialization done!!")

        self.sdn_state_input_layer = torch.nn.Linear(STATE_SPACE_DIM, 64)
        self.sdn_action_input_layer = torch.nn.Linear(ACTION_SPACE_DIM, 64)
        self.sdn_hidden_layer = torch.nn.Linear(64, 64)
        self.sdn_advantage_layer = torch.nn.Linear(64, 1)
