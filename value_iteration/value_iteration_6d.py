...
class env_quad_6d(object):
	def __init__(self, difficulty = "hard"):
		...
		self.trap_x = (-4, 0)
		self.trap_z = (0, 1)
		self.trap_theta = (-np.pi / 3, np.pi / 3)
		self.traps = None
		if (self.difficulty == "trap"):
			self.traps = np.array([self.trap_x, self.trap_z, self.trap_theta])
		########### Algorithm Setting ##########
		self.discount = 0.99
		self.threshold = 2
		self.delta = 0.3
		...
		# reward = [regula state, in goal, crashed, turnover, trap]
		self.turnover = (-1.4, 1.4)
		self.reward_list = np.array([0, 1000, -400, -800, 100], dtype = float)
	...
	def add_obstacles(self):
		...
		############## Obstacles for medium and hard environment ###################
		if (self.difficulty == "hard"):
			self.obstacles = np.array([[-2.75, -1.25, 4.25, 5.75], # [x0, x1, y0, y1]
										[0.25, 1.75, 8.0, 9.0], # <- for hard env
										# [-0.75, 0.75, 8.5, 9.5], # <- for medium env
										[2.25, 5.25, 4.25, 5.75],
										[-0.25, 0.25, 0, 2]], dtype = float)
		############################################################################

		#################### Obstacles for trap environment ########################
		elif (self.difficulty == "trap"):
			self.obstacles = np.array([[0.75, 1.25, 8, 10],
										[3, 5, 5.75, 6.25]])
	...
	def check_trap(self, s):
		for i, v in enumerate(s[:2]):
			if (v < self.traps[i][0]  or  v > self.traps[i][1]):
				return 0

		if (s[4] < self.traps[2][0]  or  s[4] > self.traps[2][1]):
			return 0
	...
	def refill_values(self):
		...
		value_file = "./value_matrix_quad_6D/value_matrix_10.npy"
...
env = env_quad_6d(difficulty = "trap")
env.algorithm_init()
# env.generate_samples_interpolate(200000)
# env.value_iteration()
